...
function SampleNextArrow(props) {
  ...
  return (
    <div
      className='absolute lg:bottom-[-35px] bottom-[-100px] right-0 lg:right-[100px] translate-y-[-50%] z-20 lg:py-[16px] py-[10px] lg:px-[32px] px-[22px] flex items-center justify-center rounded-full border border-[#00245F] hover:bg-[#00245F] duration-200 text-[#00245F] hover:text-[#fff]'
      onClick={onClick}
    >
      <ArrowRight />
    </div>
...
function SamplePrevArrow(props) {
  ...
  return (
    <div
      className='absolute lg:bottom-[-35px] bottom-[-100px] right-[130px] lg:right-[300px] translate-y-[-50%] z-10  lg:py-[16px] py-[10px] lg:px-[32px] px-[22px] flex items-center justify-center rounded-full border border-[#00245F] hover:bg-[#00245F] duration-200 text-[#00245F] hover:text-[#fff]'
      onClick={onClick}
    >
      <ArrowLeft />
    </div>
...
const Faq = () => {
  ...
  return (
    <div className='pt-[170px] pb-[101px] relative z-50 overflow-hidden lg:mx-0 mx-[10px] '>
        <div className="absolute top-[-280px] left-0 transformX-[-50%] -z-500" >
            <img src={shadow2} alt="" />
        </div>
      <Container>
        <div className="lg:flex">
          <div className="lg:w-[280px]">
            <p className='text-[24px] lg:text-start text-center font-questrial font-normal text-[#001C4A]'>
              Frequently asked questions
            </p>
          </div>
          <div>
            <h2 className='lg:text-[60px] text-[35px] lg:text-start text-center font-questrial font-normal text-[#111111]'>
      ...
      {faqs.map((item, index) => (
        <div key={index} className="border-b border-[#CCCCCC] lg:py-[30px] py-[20px]">
          <button
            onClick={() => toggle(index)}
            className="w-full flex justify-between"
          >
            <div className="flex lg:gap-x-[40px] gap-x-[10px] items-center cursor-pointer" >
              <span className="text-[#8B7D4C] font-normal lg:text-[32px] font-questrial">{item.number}</span>
            ...
            <span className="text-[#111111] text-xl font-light text-[40px]">
              {activeIndex === index ? "−" : "+"}
            </span>
          </button>
...
<div className="flex absolute lg:bottom-[8px] bottom-[-66px] right-[77px] lg:right-[220px]">
          <p className="text-[#001A72] lg:text-[20px] text-[16px] font-semibold font-manrope">
            ...
            <div key={index}>
              <div className="lg:w-[1280px] mx-auto pt-[115px]">
                <div className="flex justify-center">
                  <div>
                    <p className="lg:text-[36px] text-[18px] text-[#010205] font-manrope font-semibold leading-[160%] tracking-[-3]">
